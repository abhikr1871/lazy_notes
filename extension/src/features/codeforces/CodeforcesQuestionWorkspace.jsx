...
function CodeforcesQuestionWorkspace({ question, onBack }) {
    ...
    const getProblemDetails = (url) => {
        try {
            ...
            // Generate a unique ID (e.g. 1234A)
            const problemId = `${contestId}${index}`;
            return { contestId, index, problemId };
        } catch (e) {
            return { contestId: "", index: "", problemId: question.title.replace(/\s+/g, '') }; // Fallback
        }
    };
    ...
    const handleSaveCode = async () => {
        ...
        try {
            ...
            const payload = {
                ...
                code_snippet: code,
                language: "cpp", // Defaulting to cpp for CP, can be improved to detect
                images: []
            };
            ...
            <header className="bg-white px-4 py-3 border-b border-slate-200 flex items-center justify-between shrink-0 shadow-sm z-10">
                <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="min-w-0">
                        <h1 className="font-display font-bold text-lg text-slate-800 leading-none truncate">{index ? `${index}. ` : ''}{question.title}</h1>
